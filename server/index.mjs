...
export function createBackend({cloud=null,hosts=null,directory=resolve('server-data'),origins=['http://127.0.0.1:5177','http://127.0.0.1:5176','http://127.0.0.1:4173','http://127.0.0.1:5173']}={}){
 const simulations=new Map(),attempts=new Map();let writes=Promise.resolve();
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  if(hosts?!hosts.includes((req.headers.host??'').split(':')[0]):!/^localhost(?::\d+)?$|^127\.0\.0\.1(?::\d+)?$/.test(req.headers.host??''))return send(403,{error:'Host não autorizado.'});
  if(origin&&!origins.includes(origin))return send(403,{error:'Origem não autorizada.'});
  if(origin){res.setHeader('Access-Control-Allow-Origin',origin);res.setHeader('Vary','Origin');}
  if(req.method==='OPTIONS'){res.setHeader('Access-Control-Allow-Methods','GET,POST,PUT,OPTIONS');res.setHeader('Access-Control-Allow-Headers','Content-Type,Authorization');return send(204,{});}
  try{
   const url=new URL(req.url,'http://127.0.0.1'),parts=url.pathname.split('/').filter(Boolean);
   if(req.method==='GET'&&url.pathname==='/health')return send(200,{mode:'REMOTE_SIMULATION',storage:cloud?'supabase':'filesystem',authRequired:Boolean(cloud),version:1});
   const body=async(limit=16*1024*1024)=>{if(!req.headers['content-type']?.startsWith('application/json'))throw new Error('Use application/json.');let size=0;const chunks=[];for await(const chunk of req){size+=chunk.length;if(size>limit)throw new Error('Payload excede 16 MiB.');chunks.push(chunk);}return JSON.parse(Buffer.concat(chunks).toString());};
   if(cloud&&req.method==='POST'&&['/auth/login','/auth/refresh'].includes(url.pathname)){
    const ip=req.socket.remoteAddress||'unknown',now=Date.now();for(const [key,v] of attempts)if(v.until<now)attempts.delete(key);const limit=attempts.get(ip)||{count:0,until:now+600000};limit.count++;attempts.set(ip,limit);if(limit.count>60)return send(429,{error:'Muitas tentativas. Aguarde para tentar novamente.'});return send(200,await cloud.login(await body(8192),url.pathname==='/auth/refresh'));
   }
  ...
  }catch(error){send(error instanceof HttpError?error.status:400,{error:error instanceof SyntaxError?'JSON inválido.':error.message});}
 });return server;
}
export function startFromEnvironment(){
 const publicMode=process.env.BACKEND_PUBLIC==='true';
 const configured=Boolean(process.env.SUPABASE_URL||process.env.SUPABASE_PUBLISHABLE_KEY);
 if(publicMode&&!configured)throw new Error('Publicação exige Supabase e autenticação.');
 const cloud=configured?new SupabaseService(process.env.SUPABASE_URL,process.env.SUPABASE_PUBLISHABLE_KEY):null;
 const origins=(process.env.FRONTEND_ORIGINS||'http://127.0.0.1:5177,http://127.0.0.1:5176,http://127.0.0.1:4173,http://127.0.0.1:5173').split(',');
 const hosts=publicMode?(process.env.BACKEND_HOSTS||process.env.RENDER_EXTERNAL_HOSTNAME||'').split(',').filter(Boolean):null;
 if(publicMode&&(!hosts.length||!process.env.FRONTEND_ORIGINS||origins.some(o=>!o.startsWith('https://'))))throw new Error('Configure hosts e origens HTTPS antes de publicar.');
 const port=Number(process.env.PORT||process.env.BACKEND_PORT||8787);
 return createBackend({cloud,hosts,directory:resolve(process.env.SESSION_DATA_DIR||'server-data'),origins}).listen(port,publicMode?'0.0.0.0':'127.0.0.1',()=>console.log(`Backend REMOTE_SIMULATION porta ${port}; autenticação ${cloud?'ativa':'local'}`));
}
