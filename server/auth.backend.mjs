...
test('Supabase adapter validates tokens with Auth and uses user credentials for database access',async()=>{
 ...
 await assert.rejects(()=>service.authenticate(),/Login/);const user=await service.authenticate(`Bearer ${token}`);assert.deepEqual([...user.maps],['tabajaras']);await service.savePlanning({mapId:'tabajaras'},user);assert.equal(JSON.parse(calls.at(-1).init.body).owner_id,'a');assert.equal(calls.at(-1).init.headers.Authorization,`Bearer ${token}`);await service.loadPlanning('tabajaras',user);assert.equal(calls.at(-1).init.headers.Authorization,`Bearer ${token}`);await service.list('tabajaras',user);assert.equal(calls.at(-1).init.headers.Authorization,`Bearer ${token}`);await service.authenticate(`Bearer ${token}`);assert.equal(calls.filter(c=>c.url.endsWith('/auth/v1/user')).length,1);
