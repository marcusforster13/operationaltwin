 ...
 return <>
  ...
  <div className="workspace"><Viewport map={map} onReady={setWorld} onError={loadError} onPick={pick}/>
   <aside>
    ...
    <TelemetryPanel data={telemetry} recorded={mode==='recorded'}/>
    ...
    <div className="panel-tabs" role="tablist" aria-label="Módulos">{[['mission','Missão'],['incident','Ocorrências'],['replay','Replay']].map(([id,title])=><button role="tab" key={id} aria-selected={panel===id} onClick={()=>setPanel(id)}>{title}</button>)}</div>
    <fieldset disabled={demoRunning}>
